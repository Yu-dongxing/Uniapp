...
export default {
  ...
  onReady() {
    this.drawAssistLine('assistLine', 'green', 50, 10, 0, 0); // 矩形宽度是屏幕宽度的50%，高度是10%，从屏幕左上角开始
  },
  methods: {
    // drawAssistLine() {
    ...
    //   ctx.draw()
    // },
	drawAssistLine(canvasId, strokeStyle, rectWidthPercent, rectHeightPercent, xPercent, yPercent) {
	  const sysInfo = uni.getSystemInfoSync();
	  const screenWidth = sysInfo.screenWidth;
	  const screenHeight = sysInfo.screenHeight;
	
	  const rectWidth = screenWidth * (rectWidthPercent / 100);
	  const rectHeight = screenHeight * (rectHeightPercent / 100);
	  const x = screenWidth * (xPercent / 100);
	  const y = screenHeight * (yPercent / 100);
